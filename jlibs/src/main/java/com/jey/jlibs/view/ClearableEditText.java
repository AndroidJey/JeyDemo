...
public class ClearableEditText extends AppCompatEditText implements
     ...
     * 是否显示清除图标
     */
    private boolean showResetTextEnable = true;
    /**
     * 当输入的字符为金钱类的数字时，是否开启每三位加入逗号
     */
    private boolean addCommaEveryThreeNumber = false;
    /**
     * 是否禁止第一位输入小数点
     */
    private boolean forbidPointFirstPosition = false;
    /**
     * 是否禁止第一位输入负号
     */
    private boolean forbidMinusFirstPosition = false;
    /**
     * 是否禁止第一个不为零的数字前面不允许有0的存在
     */
    private boolean forbidZeroBeforeNotZero = false;
    /**
     * 是否允许输入Emojo表情
     */
    private boolean allowedInputEmoji = true;
    ...
    private TextWatcher watcher = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int start, int count, int after) {
            String t = ClearableEditText.this.getText().toString();
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        showResetTextEnable = a.getBoolean(R.styleable.ClearableEditText_showResetTextEnable, true);
        addCommaEveryThreeNumber = a.getBoolean(R.styleable.ClearableEditText_addCommaEveryThreeNumber, false);
        forbidPointFirstPosition = a.getBoolean(R.styleable.ClearableEditText_forbidPointFirstPosition, false);
        forbidMinusFirstPosition = a.getBoolean(R.styleable.ClearableEditText_forbidMinusFirstPosition, false);
        forbidZeroBeforeNotZero = a.getBoolean(R.styleable.ClearableEditText_forbidZeroBeforeNotZero, false);
        allowedInputEmoji = a.getBoolean(R.styleable.ClearableEditText_allowedInputEmoji, true);
        a.recycle();
        xD = getCompoundDrawables()[2];
        if (xD == null) {
            xD = getResources().getDrawable(R.mipmap.ic_edit_clear);
        }
