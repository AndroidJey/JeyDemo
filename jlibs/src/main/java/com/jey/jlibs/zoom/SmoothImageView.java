...
//        mOriginalLocationY = mOriginalLocationY - getStatusBarHeight(getContext());
    ...
    public void changeTransformState(int state) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mState == STATE_TRANSFORM_IN || mState == STATE_TRANSFORM_OUT) {
            ...
            if (mTransformStart) {
                Log.d("msgg", "mTransfrom.startScale:" + mTransfrom.startScale);
                Log.d("msgg", "mTransfrom.startScale:" + mTransfrom.endScale);
                Log.d("msgg", "mTransfrom.scale:" + mTransfrom.scale);
                Log.d("msgg", "mTransfrom.startRect:" + mTransfrom.startRect.toString());
                Log.d("msgg", "mTransfrom.endRect:" + mTransfrom.endRect.toString());
                Log.d("msgg", "mTransfrom.rect:" + mTransfrom.rect.toString());
            }
    ...
    private void startTransform(final int state) {
        ...
        if (state == STATE_TRANSFORM_IN) {
            ...
            PropertyValuesHolder scaleHolder = PropertyValuesHolder.ofFloat(
                    "scale", mTransfrom.startScale, mTransfrom.endScale);
            PropertyValuesHolder leftHolder = PropertyValuesHolder.ofFloat(
                    "left", mTransfrom.startRect.left, mTransfrom.endRect.left);
            PropertyValuesHolder topHolder = PropertyValuesHolder.ofFloat(
                    "top", mTransfrom.startRect.top, mTransfrom.endRect.top);
            PropertyValuesHolder widthHolder = PropertyValuesHolder.ofFloat(
                    "width", mTransfrom.startRect.width, mTransfrom.endRect.width);
            PropertyValuesHolder heightHolder = PropertyValuesHolder.ofFloat(
            ...
            PropertyValuesHolder alphaHolder = PropertyValuesHolder.ofInt(
                    "alpha", 255, 0);
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public synchronized void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (mTransfromAlphaListener != null)
                    mTransfromAlphaListener.onTransfromAlpha(mBgAlpha);
                if (getParent() != null) {
                    View v = (View) getParent();
                    v.setBackgroundColor(Color.argb(mBgAlpha, 0, 0, 0));
                }
                invalidate();
//                ((Activity) getContext()).getWindow().getDecorView().invalidate();
    ...
    public void setOnTransfromAlphaListener(TransfromAlphaListener mTransfromAlphaListener) {
        this.mTransfromAlphaListener = mTransfromAlphaListener;
    }
    ...
    private TransfromAlphaListener mTransfromAlphaListener;

    public static interface TransfromAlphaListener {
        void onTransfromAlpha(float alpha);
    }
}
